...
export class DataService {
    ...
    private request(options: DataServiceOptions): Observable<any> {
        ...
        const stream = this.http.request(options.method.toString(), options.url, requestOptions).pipe(
            catchError((error: any) => { return Observable.throw(error); }),
            finalize(() => { this.pendingCommandsSubject.next(--this.pendingCommandCount); })
        );
